...
5. **Reset all page and GAS changelogs** — reset every `<page-name>html.changelog.md` and `<page-name>gs.changelog.md` in `repository-information/changelogs/` and their corresponding archive files the same way. Also reset GAS `<page-name>gs.version.txt` files in `live-site-pages/gs-versions/` to `01.00g`
