...
4. This is a **session-ending action** — after pushing, close out with CODING COMPLETE as normal. **In the end-of-response block**, after the SUMMARY section (or after WORTH NOTING if present), add a `💡💡SESSION SAVED💡💡` section with the message: *"Session context saved. It's recommended to start a new session now so you don't lose the context we just created. Your next session will automatically pick up where we left off."* This reminds the user that continuing in the current session risks context compaction overwriting the freshly saved context, while a new session will load it cleanly
