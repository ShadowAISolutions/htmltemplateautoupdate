...
1. **Version bump (.gs)** — if any `.gs` file was modified, increment its `VERSION` variable by 0.01 (e.g. `"01.13g"` → `"01.14g"`) **and** update the corresponding `<page-name>gs.version.txt` in `live-site-pages/` to match (e.g. `01.13g` → `01.14g`). The `VERSION` variable in the `.gs` file is the **single source of truth** — the `gs.version.txt` file in `live-site-pages/` mirrors it for the HTML layer's GAS version polling
