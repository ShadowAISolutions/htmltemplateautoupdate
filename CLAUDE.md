...
- **Live URLs**: immediately before `✅✅CODING COMPLETE✅✅` (after ACTUAL TOTAL COMPLETION TIME), output `🔗🔗LIVE URLS (label)🔗🔗` followed by all live-site links. **Always present when the response ends with CODING COMPLETE** — never skipped for those responses, regardless of what was edited. **Skipped when the response ends with RESEARCH COMPLETE or AWAITING USER RESPONSE.** This gives the user one-click access to the live site on every coding response. Rules:
  ...
  - **Reference URLs** (always shown, every response):
    ...
    - `` `───────────────` `` (a backtick-wrapped Unicode box-drawing divider between the template and repository lines — the backticks trigger red/accent styling in the Claude Code CLI. Do NOT use bare `───────────────` without backticks inside blockquotes — it renders as plain white text. Do NOT use `---` which markdown reinterprets as a setext heading)
  ...
  - **Page URLs** (always shown, every response): list every page in `live-site-pages/` with a `Homepage:` prefix (for the root `index.html`) or a `Project Name | Homepage:` label for subdirectory `index.html` pages (e.g. `My Project | Homepage:`). The `live-site-pages/` directory is deployed as the site root, so this prefix is never part of the URL. Resolve `YOUR_ORG_NAME` and `YOUR_REPO_NAME` from the Template Variables table (using the real values from `git remote -v` on non-template repos, or the actual `ShadowAISolutions`/`htmltemplateautoupdate` values on the template repo). Rules:
    ...
    - For pages in subdirectories (e.g. `live-site-pages/my-project/index.html`), the link text shows the containing folder: `My Project | Homepage: [my-project/index.html](https://github.com/ORG/REPO/blob/main/live-site-pages/my-project/index.html) →` [YOUR_ORG_NAME.github.io/YOUR_REPO_NAME/my-project/](https://YOUR_ORG_NAME.github.io/YOUR_REPO_NAME/my-project/)
    - **Page-level dividers** — when there are multiple pages, separate each page entry with `` > `───────────────` `` (backtick-wrapped, inside the blockquote). This uses the same divider character as the section dividers, giving all dividers the red/accent CLI styling. The visual hierarchy is maintained by context — section dividers separate structural groups (template → repo → pages), while page dividers separate individual entries within the pages group
  ...
  - **CLI red/accent text technique**: the Claude Code CLI renders backtick-wrapped inline code (`` ` ``) inside blockquotes with a red/accent color and a subtle bordered background. Use this to make divider lines visually distinct: write `` > `───────────────` `` (backtick-wrapped box-drawing characters inside a blockquote). Without backticks, the same characters render as plain white text inside blockquotes. This technique can be applied to any text inside blockquotes that should stand out — wrap it in backticks to trigger the accent styling. Note: this is a Claude Code CLI rendering behavior, not standard markdown — the styling does not appear on GitHub or other markdown renderers
