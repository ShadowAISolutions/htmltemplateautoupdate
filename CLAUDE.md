...
17. **Page changelog** — if any embedding HTML page in `live-site-pages/` was modified (directly or indirectly via `.gs` changes), add a user-facing entry to the page's `<page-name>.changelog.md` in the same directory. These changelogs are **user-facing** — they describe what a visitor to the webpage would notice, not internal/backend details. **Writing style**: describe changes from the user's perspective. Say "Faster page loading" not "Optimized database queries". Say "Fixed login button not responding on mobile" not "Fixed event listener delegation in auth.js". Never mention file names, function names, commit SHAs, deployment IDs, or internal architecture. Categories follow Keep a Changelog: `### Added`, `### Changed`, `### Fixed`, `### Removed`. Format: `- Description` (no timestamps per entry). **Versioned release sections** — on the push commit, move entries from `## [Unreleased]` into a new version section using the page version (`w`) with the repo version for developer cross-reference: `## [vXX.XXw] (vXX.XXr) — YYYY-MM-DD HH:MM:SS AM/PM EST`. The repo version in parentheses lets the developer trace which push introduced the page change. No SHA backfill — these are user-facing, not developer-facing. `## [Unreleased]` is then left empty. **Archive rotation** — same 50-section rotation logic as the repo CHANGELOG (documented in `repository-information/CHANGELOG-archive.md`). Rotate to `<page-name>.changelog-archive.md`. **Capacity counter** — update the `` `Sections: X/50` `` counter after creating the new version section. **When no user-visible change occurred** — if the HTML/GAS change is purely internal (e.g. code refactoring with no visible effect), skip the page changelog entry. Only add entries when the user's experience of the page is affected. **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`** — when skipped, keep page changelogs clean with `*(No changes yet)*` under `## [Unreleased]`. **When `TEMPLATE_DEPLOY` is switched `Off`**, reset all page changelogs and their archives the same way as the repo CHANGELOG
...
- **Reset `repository-information/CHANGELOG.md`** — remove all versioned release sections and all entries/category headings under `[Unreleased]`, leaving a fresh template (header, version suffix note, and an empty `## [Unreleased]` section with `*(No changes yet)*`). Also reset `repository-information/CHANGELOG-archive.md` — remove all archived sections and restore the `*(No archived sections yet)*` placeholder. **Also reset all page changelogs** — reset every `<page-name>.changelog.md` in `live-site-pages/` and their corresponding `<page-name>.changelog-archive.md` the same way. This gives the repo a clean history starting point
