...
> **Rebase before push commit** — when a second push is needed in the same session (the previous push's branch was merged and deleted by the workflow — see Pre-Push #5 for detection), complete all intermediate commits first, then fetch and rebase onto `origin/main`, and only then begin the push commit cycle. **The working tree must be clean before the rebase** — if any uncommitted changes exist, commit them as intermediate commits first. This eliminates the stash/rebase/pop pattern entirely. Sequence: (1) commit all pending work (staged and unstaged), (2) `git fetch origin main`, (3) `git rebase origin/main`, (4) start the push commit cycle (version bump, CHANGELOG, README timestamp, etc.).
