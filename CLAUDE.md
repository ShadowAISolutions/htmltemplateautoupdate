...
> **Rebase before push commit** — when a second push is needed in the same session (the previous push's branch was merged and deleted by the workflow), complete all intermediate commits first, then fetch and rebase onto `origin/main`, and only then begin the push commit cycle. This eliminates the stash/rebase/pop pattern — the working tree is always clean when the rebase happens. Sequence: (1) commit all intermediate work, (2) `git fetch origin main`, (3) `git rebase origin/main`, (4) start the push commit cycle (version bump, CHANGELOG, README timestamp, etc.).
