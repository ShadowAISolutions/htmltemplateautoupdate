...
> **Rebase before push commit** — when a second push is needed in the same session (the previous push's branch was merged and deleted by the workflow — see Pre-Push #5 for detection), complete all intermediate commits first, then fetch and rebase onto `origin/main`, and only then begin the push commit cycle. **The working tree must be clean before the rebase** — if any uncommitted changes exist, commit them as intermediate commits first. This eliminates the stash/rebase/pop pattern entirely. Sequence: (1) commit all pending work (staged and unstaged), (2) `git fetch origin main`, (3) `git rebase origin/main`, (4) start the push commit cycle (version bump, CHANGELOG, README timestamp, etc.). **CRITICAL: check rebase need BEFORE starting the push commit cycle, not during Pre-Push.** If a push already succeeded earlier in this session, run `git ls-remote origin <branch-name>` to check whether the branch was merged/deleted BEFORE making the push commit. If the branch is gone, fetch and rebase first. Making the push commit before rebasing causes SHA backfill invalidation — rebase rewrites commit SHAs, so any SHA captured before the rebase becomes stale and must be re-captured and re-backfilled. The correct order is always: rebase → push commit → SHA backfill → push.
