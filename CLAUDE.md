...
5. **Push-once enforcement** — verify that no push has already been made to this `claude/*` branch in this session. If a push already succeeded earlier in the session, STOP — do not push again (see Deployment Flow rules). There are two exceptions:
   - **Workflow completed (branch deleted)** — run `git ls-remote origin <branch-name>` to check if the auto-merge workflow has already merged and deleted the branch. If the branch no longer exists on the remote, the workflow finished cleanly — pushing to the same branch name is safe because it creates the branch fresh with no queued-workflow collision. When the push-once block triggers, **always present this check as an option to the user** (e.g. "I can check if the workflow already finished and the branch was deleted — if so, we can push safely"). If the user agrees, run the check; if the branch is gone, proceed with the push. If the branch still exists, wait ~15 seconds and check again (up to 3 retries) before concluding the workflow hasn't finished yet
