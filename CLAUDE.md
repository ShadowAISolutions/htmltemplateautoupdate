...
2. **Version bump (version.txt + meta tag)** — if any embedding HTML page in `live-site-pages/` was modified, increment the version in its `<page-name>.version.txt` by 0.01 (e.g. `|v01.01w|` → `|v01.02w|`) **and** update the `<meta name="build-version" content="|vXX.XXw|">` tag in the HTML to match. The version.txt file is the **single source of truth** for the auto-refresh polling — the meta tag is informational only (not read by the polling logic), so changing only version.txt still triggers a reload correctly. **Skip if Template Repo Guard applies (see above)**
...
- **Activate with timestamp**: also fill the third field with the start time (e.g. `maintenance|v01.02w|2026-02-26 10:00:00 PM EST`). The timestamp is displayed below the logo on the overlay as "Since: ..."
