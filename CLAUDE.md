...
- **Activate with timestamp**: also fill the third field with the start time (e.g. `maintenance|v01.02w|2026-02-26 10:00:00 PM EST`). The timestamp is displayed below the logo on the overlay as "As of: 10:00:00 PM EST 02/26/2026" — the JS reformats the `YYYY-MM-DD TIME` input into `TIME MM/DD/YYYY` display format. The "As of: " prefix is only added for pure timestamps (matching `YYYY-MM-DD ...`); custom text or mixed content in the third field is displayed verbatim (e.g. `maintenance|v01.02w|Back online soon!` → "Back online soon!")
