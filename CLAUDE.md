...
>   - When `TEMPLATE_DEPLOY` = `On`: version bumps ARE performed — Pre-Commit items #1 (`.gs` version bump), #2 (version.txt bump), #3 (version.txt single source), #5 (STATUS.md), #6 (ARCHITECTURE.md version nodes), #7 (CHANGELOG.md), #9 (version prefix in commit message), and #16 (repo version bump + SHA backfill) all run normally, so that deployed pages auto-refresh after changes and the CHANGELOG tracks version history. Item **#14 (QR code generation)** is still skipped regardless — QR codes use placeholder URLs on the template
>   - When `TEMPLATE_DEPLOY` = `Off`: all version bumps are skipped — items #1, #2, #3, #5, #6 (version-bump portion), #7, #9, #14, and #16 are all skipped (the original behavior). Additionally, if any versions were previously bumped above `v01.00w` / `"01.00g"` / `v01.00r`, **reset them to initial values**: version.txt files → `|v01.00w|`, HTML meta tags → `v01.00w`, `.gs` VERSION → `"01.00g"`, `repository.version.txt` → `v01.00r`, ARCHITECTURE.md VERTXT nodes → `(|v01.00w|)`, STATUS.md versions → initial. **Also reset `repository-information/CHANGELOG.md`** — remove all versioned release sections (`## [vXX.XX*]` sections), remove all entries and category headings under `## [Unreleased]`, replace with `*(No changes yet)*`, and remove the date from the section header (→ `## [Unreleased]`). This ensures forks inherit clean starting versions and a blank change history
...
> - Pre-Commit items #0, #4, #8, #10, #11, #12, #13, #15 still apply normally (plus #1, #2, #3, #5, #6, #9, #16 when `TEMPLATE_DEPLOY` = `On`)
...
> - `TEMPLATE_DEPLOY` = `On`: only #14 (QR code) is skipped. Items #1, #2, #3, #5, #6 (version-bump portion), #7, #9, and #16 run normally — version bumps are needed so deployed pages auto-refresh, and CHANGELOG tracks version history
> - `TEMPLATE_DEPLOY` = `Off`: items #1, #2, #3, #5, #6 (version-bump portion), #7, #9, #14, and #16 are **all skipped** — do NOT bump versions, update version-tracking files, add CHANGELOG entries, use version prefixes in commit messages, or generate QR codes. Additionally, reset the CHANGELOG to clean state (see Template Repo Guard). Proceed directly to the items that still apply (#0, #4, #8, #10, #11, #12, #13, #15)
>
...
7. **CHANGELOG.md** — every user-facing change must have an entry under `## [Unreleased]` in `repository-information/CHANGELOG.md`, grouped under the appropriate Keep a Changelog category: `### Added` (new features), `### Changed` (changes to existing functionality), `### Fixed` (bug fixes), `### Deprecated` (soon-to-be removed features), `### Removed` (removed features), `### Security` (vulnerability fixes). Only include categories that have entries — do not add empty category headings. Each entry must include an EST timestamp down to the second (format: `` `YYYY-MM-DD HH:MM:SS EST` — Description``). The `[Unreleased]` section header must also show the date/time of the most recent entry. **Timestamps must be real** — run `TZ=America/New_York date '+%Y-%m-%d %H:%M:%S EST'` to get the actual current time; never fabricate or increment timestamps. **Versioned release sections** — on every commit, move all entries from `## [Unreleased]` into a new version section. The repo version (Pre-Commit #16) always bumps, so every commit produces a version section. Format: `## [vXX.XXr] — YYYY-MM-DD` (repo-only changes), `## [vXX.XXr vXX.XXg] — YYYY-MM-DD` (repo + GAS), `## [vXX.XXr vXX.XXw] — YYYY-MM-DD` (repo + page), or `## [vXX.XXr vXX.XXg vXX.XXw] — YYYY-MM-DD` (all three). Order in header: `r` first, then `g`, then `w`. The version section keeps its category groupings intact. `## [Unreleased]` is then left empty (no placeholder text needed — it will accumulate entries from the next commit onward). Latest versions appear first (reverse chronological). **Commit SHA in header** — the SHA is added retroactively by the next commit (Pre-Commit #16 backfill) since it's not known at commit time. Format after backfill: `## [vXX.XXr] — YYYY-MM-DD — \`SHORT_SHA\``. **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`** — when skipped, the CHANGELOG must stay clean with `*(No changes yet)*` under a bare `## [Unreleased]` so forks start with a blank history. When `TEMPLATE_DEPLOY` = `On`, CHANGELOG entries are added normally to track version changes on the deployed template. **When `TEMPLATE_DEPLOY` is switched from `On` to `Off`**, reset the CHANGELOG: remove all version sections, remove all entries and category headings under `## [Unreleased]`, replace with `*(No changes yet)*`, and remove the date from the section header (see Template Repo Guard)
8. **README.md structure tree** — if files or directories were added, moved, or deleted, update the ASCII tree in `README.md`
9. **Commit message format** — every commit message must start with the repo version prefix `v{REPO_VERSION}` (e.g. `v01.01r`). If `.gs` or HTML versions were also bumped, append those prefixes in order: `r`, `g`, `w` (e.g. `v01.05r v01.14g v01.02w Fix bug`). **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`**
...
16. **Repo version bump** — on every commit, increment the version in `repository-information/repository.version.txt` by 0.01 (e.g. `v01.00r` → `v01.01r`). The `r` suffix stands for "repository" and distinguishes it from page versions (`w`) and GAS versions (`g`). This version is the **single source of truth** for the overall repository version — it bumps on every commit regardless of which files changed. **Backfill previous commit SHA** — if the most recent versioned release section in `repository-information/CHANGELOG.md` has no commit SHA in its header, fill it in by running `git log -1 --format='%h'` (this returns the SHA of the commit that created that section — the SHA wasn't known at commit time, so it's added retroactively by the next commit). **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`** — when skipped, reset `repository.version.txt` to `v01.00r` so forks inherit a clean starting version
...
- Every commit starts with the repo version prefix (`v01.XXr`) since repo version always bumps
- When `.gs` or HTML versions are also bumped, append them in order: `r`, `g`, `w`
- Example: `v01.05r Fix typo in CLAUDE.md` (repo-only change)
- Example: `v01.06r v01.19g Fix sign-in popup to auto-close after authentication`
- Example: `v01.07r v01.19g v01.12w Add auth wall with build version bump`
...
- **Reset `repository-information/CHANGELOG.md`** — remove all versioned release sections and all entries/category headings under `[Unreleased]`, leaving a fresh template (header, version suffix note, and an empty `## [Unreleased]` section with `*(No changes yet)*`). This gives the repo a clean history starting point
