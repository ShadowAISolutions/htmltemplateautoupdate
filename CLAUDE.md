...
>   - When `TEMPLATE_DEPLOY` = `On`: version bumps ARE performed — Pre-Commit items #1 (`.gs` version bump), #2 (version.txt bump), #3 (version.txt single source), #5 (STATUS.md), #6 (ARCHITECTURE.md version nodes), and #9 (version prefix in commit message) all run normally, so that deployed pages auto-refresh after changes. Items **#7 (CHANGELOG.md)** and **#14 (QR code generation)** are still skipped regardless — the CHANGELOG must stay clean with `*(No changes yet)*` so forks start with a blank history, and QR codes use placeholder URLs on the template
>   - When `TEMPLATE_DEPLOY` = `Off`: all version bumps are skipped — items #1, #2, #3, #5, #6 (version-bump portion), #7, #9, and #14 are all skipped (the original behavior). Additionally, if any versions were previously bumped above `v01.00w` / `"01.00g"`, **reset them to initial values**: version.txt files → `|v01.00w|`, HTML meta tags → `v01.00w`, `.gs` VERSION → `"01.00g"`, ARCHITECTURE.md VERTXT nodes → `(|v01.00w|)`, STATUS.md versions → initial. This ensures forks inherit clean starting versions
...
> - `TEMPLATE_DEPLOY` = `On`: only #7 (CHANGELOG) and #14 (QR code) are skipped. Items #1, #2, #3, #5, #6 (version-bump portion), and #9 run normally — version bumps are needed so deployed pages auto-refresh
> - `TEMPLATE_DEPLOY` = `Off`: items #1, #2, #3, #5, #6 (version-bump portion), #7, #9, and #14 are **all skipped** — do NOT bump versions, update version-tracking files, add CHANGELOG entries, use version prefixes in commit messages, or generate QR codes. Proceed directly to the items that still apply (#0, #4, #8, #10, #11, #12, #13, #15)
>
...
2. **Version bump (version.txt + meta tag)** — if any embedding HTML page in `live-site-pages/` was modified, increment the version in its `<page-name>.version.txt` by 0.01 (e.g. `|v01.01w|` → `|v01.02w|`) **and** update the `<meta name="build-version" content="vXX.XXw">` tag in the HTML to match (the meta tag uses just the version string, no pipes). The version.txt file is the **single source of truth** for the auto-refresh polling — the meta tag is informational only (not read by the polling logic), so changing only version.txt still triggers a reload correctly. **Skip on template repo when `TEMPLATE_DEPLOY` = `Off`**
