...
6. **README live site link** — check if `README.md` still contains the placeholder text (`You are currently using the **YOUR_REPO_NAME**...`). If so, replace it with: `**Live site:** [YOUR_ORG_NAME.github.io/YOUR_REPO_NAME](https://YOUR_ORG_NAME.github.io/YOUR_REPO_NAME)` (resolved values). The QR code `<img>` tag below the link must be preserved — it references `repository-information/qr-code.png` via a relative path that works on all forks without modification. The auto-merge workflow regenerates the QR PNG automatically when README.md changes
