...
- **Context compaction recovery**: when the conversation context has been compacted (indicated by a session summary replacing earlier messages), treat it as a fresh session start. **Re-read the actual rules in CLAUDE.md** before producing any output — do not rely on patterns, formats, or behaviors carried over from the compacted summary. Summaries describe what was done but do not encode the rules that governed those actions; defaulting to "how the summary described it" instead of "what CLAUDE.md currently says" causes drift (e.g. wrong URL formats, skipped checklist steps, stale variable values). Specifically: re-execute the full Session Start Checklist (all Always Run items), re-derive all output formatting from the current CLAUDE.md rules (Chat Bookends, URL display patterns, timestamp formats, end-of-response block structure), and re-read any referenced files (TODO.md, REMINDERS.md, Template Variables table) rather than assuming the summary's snapshot is current
...
The Mermaid diagram in `repository-information/ARCHITECTURE.md` contains nodes that display version strings. When a version is bumped in html.version.txt, **all** nodes showing that version must be updated.

### Version-bearing nodes

| Node ID | What it represents | Example text | Tracks |
|---------|--------------------|-------------|--------|
| `INDEX` | `index.html` | `INDEX["index.html"]` | The embedding page (version lives in html.version.txt, not in the HTML) |
| `VERTXT` | `indexhtml.version.txt` | `VERTXT["indexhtml.version.txt\n(\|v01.00w\|)"]` | The single source of truth for the page version |
| `TPL` | `AutoUpdateOnlyHtmlTemplate.html` | `TPL["...\n(\|v01.00w\| — never bumped)"]` | Frozen at `\|v01.00w\|` — never changes |

### Why the miss happens
The `html.version.txt` file gets bumped by Pre-Commit item #2, but the VERTXT *Mermaid node* is a separate representation of the same version. It's easy to bump the file and forget the diagram node. Always check the VERTXT node when bumping a version.
...
- **Unaffected/Affected URLs (split into two sections)**: the URLs are split into an **unaffected** group and an **affected** group, appearing in different positions within the end-of-response block. **Both are skipped when the response ends with RESEARCH COMPLETE or AWAITING USER RESPONSE.** Rules:
  ...
  - **Format**: each entry is a two-line pair separated by blank lines. Line 1: backtick-wrapped label at top level (no `>`, red/accent in CLI). Line 2: the URL in a blockquote (`>`). In the affected group: `` `✏️ Homepage (v01.13w)` `` then `> [index.html](https://github.com/.../blob/main/live-site-pages/index.html) →` [ShadowAISolutions.github.io/htmltemplateautoupdate/](https://ShadowAISolutions.github.io/htmltemplateautoupdate/). In the unaffected group: `` `Homepage` `` then `> [index.html](https://github.com/.../blob/main/live-site-pages/index.html) → ...`. For subpages in the affected group: `` `✏️ My Project | Homepage (v01.05w)` `` then `> [my-project/index.html](https://github.com/.../blob/main/live-site-pages/my-project/index.html) →` [ORG.github.io/REPO/my-project/](https://ORG.github.io/REPO/my-project/)
...
| `🔗✏️AFFECTED URLS✏️🔗` | Every response with CODING COMPLETE | After TODO — affected pages with version (e.g. `✏️ Homepage (v01.02w)`), or placeholder if none (never skipped) | — | — |
