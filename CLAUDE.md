...
> **Output formatting** — see `.claude/rules/chat-bookends.md` for mandatory response formatting: CODING PLAN → CODING START → work → CODING COMPLETE with timestamps on every response.
...
> **Single commit per interaction** — each user interaction (one back-and-forth turn) should produce **one commit** whenever possible. Do not split work into an intermediate commit + push commit when both can be combined. The push commit should contain all changes for the interaction: the user's requested edits, version bumps, CHANGELOG, README timestamp — everything in one commit. **Intermediate commits are only justified when**: (a) the user explicitly requests multiple separate commits, (b) work is being committed as a checkpoint before a risky operation (e.g. a complex rebase with expected conflicts), or (c) a prior push in the same session already merged and a new push is needed (the prior push's commit is already on `main`). The "push commit vs. intermediate commit" distinction below still applies for checklist item scoping — but the goal is to make every commit the push commit by default.
