...
6. **README live site link** — check if `README.md` still contains the placeholder text (`You are currently using the **YOUR_REPO_NAME**...`). If so, replace the entire placeholder block — from the `You are currently using the **` line through the QR code `</p>` closing tag (inclusive) — with: `**Live site:** [YOUR_ORG_NAME.github.io/YOUR_REPO_NAME](https://YOUR_ORG_NAME.github.io/YOUR_REPO_NAME)` (resolved values) followed by `<p align="center"><img src="repository-information/qr-code.png" alt="QR code to live site" width="200"></p>`. If the QR code `<p>` block is not present (older forks created before the QR code was added to the template), replace only through the `Update your code...` line and add the QR `<p>` block after the live site link. The `<img>` tag stays the same on forks — only the PNG content changes (Pre-Commit Checklist #14 regenerates it with the fork's live site URL)
