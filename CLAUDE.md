...
>   - When `TEMPLATE_DEPLOY` = `On`: version bumps ARE performed — Pre-Commit items #1 (`.gs` version bump), #2 (version.txt bump), #3 (version.txt single source), #5 (STATUS.md), #6 (ARCHITECTURE.md version nodes), #7 (CHANGELOG.md), #9 (version prefix in commit message), #16 (repo version bump + SHA backfill), and #17 (page changelog) all run normally, so that deployed pages auto-refresh after changes and changelogs track version history. Item **#14 (QR code generation)** is still skipped regardless — QR codes use placeholder URLs on the template
>   - When `TEMPLATE_DEPLOY` = `Off`: all version bumps are skipped — items #1, #2, #3, #5, #6 (version-bump portion), #7, #9, #14, #16, and #17 are all skipped (the original behavior). Additionally, if any versions were previously bumped above `v01.00w` / `"01.00g"` / `v01.00r`, **reset them to initial values**: version.txt files → `|v01.00w|`, HTML meta tags → `v01.00w`, `.gs` VERSION → `"01.00g"`, `repository.version.txt` → `v01.00r`, ARCHITECTURE.md VERTXT nodes → `(|v01.00w|)`, STATUS.md versions → initial. **Also reset `repository-information/CHANGELOG.md`** — remove all versioned release sections (`## [vXX.XX*]` sections), remove all entries and category headings under `## [Unreleased]`, replace with `*(No changes yet)*`, and remove the date from the section header (→ `## [Unreleased]`). Also reset `repository-information/CHANGELOG-archive.md` — remove all archived sections and restore the `*(No archived sections yet)*` placeholder. **Also reset all page and GAS changelogs** — reset every `<page-name>.changelog.md` in `live-site-pages/` and `googleAppsScripts/*/` and their corresponding `<page-name>.changelog-archive.md` the same way (clear version sections, restore `*(No changes yet)*` / `*(No archived sections yet)*`). Also reset GAS `<page-name>.version.txt` files in `googleAppsScripts/*/` to `01.00g`. This ensures forks inherit clean starting versions and a blank change history
...
> - Pre-Commit items #0, #4, #8, #10, #11, #12, #13, #15 still apply normally (plus #1, #2, #3, #5, #6, #9, #16, #17 when `TEMPLATE_DEPLOY` = `On`)
...
**No version bumps** — initialization never bumps any version: not `version.txt` files, not `repository.version.txt`, not `.gs` VERSION, not HTML meta tags — no version-tracking files of any kind. It deploys whatever versions already exist. This applies on both the template repo and forks.
...
- Each GAS project also has a `<page-name>.version.txt` that mirrors the `VERSION` variable value (e.g. `01.00g`). This file is bumped alongside `VERSION` by Pre-Commit #1
...
10. **Create GAS version file and changelog** — if this page has a GAS project, copy `AutoUpdateOnlyHtmlTemplate.version.txt` into the GAS project directory as `<page-name>.version.txt` (initial value `01.00g`). Also copy `AutoUpdateOnlyHtmlTemplate.changelog.md` and `AutoUpdateOnlyHtmlTemplate.changelog-archive.md` as `<page-name>.changelog.md` and `<page-name>.changelog-archive.md`, replacing `YOUR_PROJECT_TITLE` with the project name
11. **Add developer branding** — ensure `<!-- Developed by: DEVELOPER_NAME -->` is the last line of the HTML file
12. **Create page changelog** — copy `live-site-templates/AutoUpdateOnlyHtmlTemplate.changelog.md` into the page's directory, renaming to `<page-name>.changelog.md`. Replace `YOUR_PROJECT_TITLE` with the page's human-readable title and update the archive link filename. Also copy `AutoUpdateOnlyHtmlTemplate.changelog-archive.md` as `<page-name>.changelog-archive.md` and update its title and changelog link filename
...
│   ├── index.changelog.md       # User-facing changelog for the page
│   ├── index.changelog-archive.md # Older changelog sections (rotated)
│   └── sounds/
│       └── Website_Ready_Voice_1.mp3
```
For pages that live directly in `live-site-pages/` (not in a subdirectory), the version file, changelog files, and `sounds/` folder sit alongside the HTML file (e.g. `live-site-pages/index.html` + `live-site-pages/index.version.txt` + `live-site-pages/index.changelog.md`).
...
- **Reset `repository-information/CHANGELOG.md`** — remove all versioned release sections and all entries/category headings under `[Unreleased]`, leaving a fresh template (header, version suffix note, and an empty `## [Unreleased]` section with `*(No changes yet)*`). Also reset `repository-information/CHANGELOG-archive.md` — remove all archived sections and restore the `*(No archived sections yet)*` placeholder. **Also reset all page and GAS changelogs** — reset every `<page-name>.changelog.md` in `live-site-pages/` and `googleAppsScripts/*/` and their corresponding `<page-name>.changelog-archive.md` the same way. Also reset GAS `<page-name>.version.txt` files to `01.00g`. This gives the repo a clean history starting point
